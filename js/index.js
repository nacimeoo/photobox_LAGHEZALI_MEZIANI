(() => {
  ...
  function loadRessource(uri) {
    const BASE_URL3 = "https://webetu.iutnc.univ-lorraine.fr";
    const url = `${BASE_URL3}${uri}`;
    return fetch(url, { credentials: "include" }).then((response) => {
      if (!response.ok) {
        throw new Error(`Erreur ${response.status}`);
      }
  ...
  var BASE_URL2 = "https://webetu.iutnc.univ-lorraine.fr";
  function displayPicture(data) {
    const templateScript = document.getElementById("photoTemplate");
